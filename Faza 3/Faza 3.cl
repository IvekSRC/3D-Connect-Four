...
(defun potezX1 (row column lista simbol)
  (if (= row 0) (setq lista (cons (cons (car (car lista)) (potezX2 column (cdr (car lista)) simbol)) (cdr lista))) (setq lista (cons (car lista) (potezX1 (1- row) column (cdr lista) simbol))))
) 
...
(defun igracProtivIgraca ()
  (loop while (> krajIgre 0) do 
        ...
        (if (= (mod naPotezu 2) 0) 
          (setq playground (potezX1 vrsta kolona playground 'X)) 
          (setq playground (potezX1 vrsta kolona playground 'O)))
        (stampaj playground)
        (setq naPotezu (1+ naPotezu))
  )
)
...
(defun MinMax (state move alpha beta currentDepth isMyMove)
    ...
    (cond
        ((zerop currentDepth) (list move (evaluate move isMyMove state)))
        (t
            (setq result
                     (if isMyMove
            ...
            (cond
		 ((null moguciSledeciPotezi) (list move (if isMyMove '1000 '-1000)))
...
(defun MaxPlay (movesList bestMove depth alpha beta isMyMove previousState)
    ...
    (cond 
        ...
        (t 
            (let*
                (                
		    (previousMoveState (potezX1 (car (car movesList)) (cadr (car movesList)) previousState simbol))
...
(defun evaluate (move isMyMove state)
    (random 100)
)
...
(loop for i from 1 to (1- n) do
   (setq playground (potezX1 0 0 playground 'X))
   (setq playground (potezX1 0 1 playground 'X))
   (setq playground (potezX1 0 2 playground 'X))
   (setq playground (potezX1 0 3 playground 'X))
   (setq playground (potezX1 1 0 playground 'X))
   (setq playground (potezX1 1 1 playground 'X))
   (setq playground (potezX1 1 2 playground 'X))
   (setq playground (potezX1 1 3 playground 'X))
   (setq playground (potezX1 2 0 playground 'X))
   (setq playground (potezX1 2 1 playground 'X))
   (setq playground (potezX1 2 2 playground 'X))
   (setq playground (potezX1 2 3 playground 'X))
   (setq playground (potezX1 3 0 playground 'X))
   (setq playground (potezX1 3 1 playground 'X))
   (setq playground (potezX1 3 2 playground 'X))
   (setq playground (potezX1 3 3 playground 'X))
)
...
;; Sledecim pozivom treba da se generise random vrednost iz liste mogucih poteza ((0 3) (1 2) (2 2) (3 0))
(setq rez (MinMax playground previousMove -500 500 2 t))
(stampaj playground)
(generisiSveMogucePoteze playground)